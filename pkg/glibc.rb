name :glibc
version '2.29'
archive "https://ftp.gnu.org/gnu/#{name}/#{name}-#{version}.tar.xz"
signature "#{archive}.sig"

on_build do |package|
  mkdir_p package.build_path.join('glibc-build')

  cd package.build_path.join('glibc-build') do
    sh <<~EOS
...
on_install do |package|
  ...
  cd package.build_path.join('glibc-build') do
    make :install, 'install_root' => os_root
  end

  cd package.build_path do
    sh <<~EOS
      install -m644 nscd/nscd.conf "#{os_root}/etc/nscd.conf"
      install -m644 posix/gai.conf "#{os_root}/etc/gai.conf"
    EOS
  end
end
...
files %w(
  ...
  /usr/lib/libmvec-2.29.so
  /usr/lib/libmvec.a
  /usr/lib/libmvec.so
  /usr/lib/libmvec.so.1
  /usr/lib/libmvec_nonshared.a
  /usr/lib/libnsl-2.29.so
  /usr/lib/libnsl.so.1
  /usr/lib/libnss_compat-2.29.so
  /usr/lib/libnss_compat.so
  /usr/lib/libnss_compat.so.2
  /usr/lib/libnss_db-2.29.so
