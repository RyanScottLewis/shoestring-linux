name      'ncurses'
version   '6.1'
archive   "https://ftp.gnu.org/pub/gnu/#{name}/#{name}-#{version}.tar.gz"
...
on_install do |package|
  ...
  major   = package.version.split(?.).first

  cd(package.build_path) do
    make :install, 'DESTDIR' => os_root

    %w[ncurses ncurses++ form panel menu].each do |lib|
      sh "echo 'INPUT(-l#{lib}w)' > '#{os_root}/usr/lib/lib#{lib}.so'"
      ln_sf "#{lib}w.pc", "#{os_root}/usr/lib/pkgconfig/#{lib}.pc"
    end

    %w[tic tinfo].each do |lib|
      sh "echo 'INPUT(libncursesw.so.#{major})' > '#{os_root}/usr/lib/lib#{lib}.so'"
      ln_sf "libncursesw.so.#{major}", "#{os_root}/usr/lib/lib#{lib}.so.#{major}"
      ln_sf 'ncursesw.pc', "#{os_root}/usr/lib/pkgconfig/#{lib}.pc"
    end

    sh "echo 'INPUT(-lncursesw)' > '#{os_root}/usr/lib/libcursesw.so'"
    ln_sf 'libncurses.so', "#{os_root}/usr/lib/libcurses.so"
  end
end
